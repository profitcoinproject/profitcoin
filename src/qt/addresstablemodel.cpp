...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == Qt::FontRole)
    {
        ...
        {
            font = GUIUtil::profitcoinAddressFont();
        }
        return font;
    }
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CProfitcoinAddress(strTemp).Get(), value.toString().toStdString());
            }
        ...
        case Address:
            ...
            if(CProfitcoinAddress(rec->address.toStdString()) == CProfitcoinAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CProfitcoinAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CProfitcoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CProfitcoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
{
    // Update address book model from Profitcoin core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if (type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CProfitcoinAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CProfitcoinAddress(strAddress).Get(), strLabel);
        }
    }
    }else if(type == Receive)
    {
        ...
        } else
        {
            ...
            strAddress = CProfitcoinAddress(newKey.GetID()).ToString();
...
QString AddressTableModel::labelForAddress(const QString &address) const
{
    {      
        ...
        {
            CProfitcoinAddress address_parsed(sAddr);
